    ...
    if os.path.exists(path_gt):
        ...
        for file in tqdm(flist):
            try:
                ...
                gt = cv2.imread(path_gt + file[:-4] + '_gt.png') #C1

                # gt = cv2.imread(path_gt + file[:-5] + 'forged.tif') #Coverage
                
                # gt = cv2.imread(path_gt + file) # NC16

                # gt = cv2.imread(path_gt + file[:-4]+'_edgemask.png') #Columbia
...
if __name__ == "__main__":
    save_path = 'xxxxxxxxxxxxxx'
    path_gt = 'xxxxxxxxxxxxxxx'
    record_txt = r"./test_out/CBM-Net.txt"
    with open(record_txt,"a") as f:
        f.writelines("\n")

    auc,f1,iou=evaluate(save_path,path_gt,"xxxx",record_txt)